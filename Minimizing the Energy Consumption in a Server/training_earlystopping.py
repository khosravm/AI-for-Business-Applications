...
epsilon = .3  # it means we will do 30% exploration
number_actions = 5
...
if (env.train):
    ...
    for epoch in range(1, number_epochs):
        ...
        game_over = False
        current_state, _, _ = env.observe() # it means that we just need and so take first output of observe()
        timestep = 0
        # STARTING THE LOOP OVER ALL THE TIMESTEPS (1 Timestep = 1 Minute) IN ONE EPOCH
        while ((not game_over) and timestep <= 5 * 30 * 24 * 60):  # 5 * 30 * 24 * 60: the total number of minutes in 5 months; each epoch take longs 5 month
            # PLAYING THE NEXT ACTION BY EXPLORATION (occurs 30% of times)
            if np.random.rand() <= epsilon:   # 30% of times
            ...
            # PLAYING THE NEXT ACTION BY INFERENCE[= predicting the output] (occurs 70% of times)
            else:
            ...
            total_reward += reward
            # STORING THIS NEW TRANSITION INTO THE MEMORY (we have to store it in to the memory because our model is implemented with experience replay)
            ...
            loss += model.train_on_batch(inputs, targets)   # train_on_batch: from Model class of keras.models library
            timestep += 1
